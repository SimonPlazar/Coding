...
Artwork::Artwork(std::string title, std::string description, double price, int year, Artist* artist, double width, double height, double depth) :
                title(std::move(title)), description(std::move(description)), price(price), year(year),
                artist(artist), dimension(new Dimension(width, height, depth)){
}
