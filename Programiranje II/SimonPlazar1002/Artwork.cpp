...
Artwork::Artwork(std::string title, std::string description, double price, int year, std::shared_ptr<Artist> artist)
        : title(std::move(title)), description(std::move(description)), price(price), year(year),
...
void Artwork::setArtist(std::shared_ptr<Artist> x) {
    Artwork::artist = std::move(x);
}
...
bool Artwork::isExpensive(const std::shared_ptr<Artwork> &second) const {
...
Artwork::Artwork() : title(""), description(""), price(0), year(0), artist(std::make_shared<Artist>()) {}
